...
const CourseCard = ({cardData}) => {
    ...
    useEffect( async () => {
        const imgUrl = cardData.thumbnail.replace("\\\\","/\\/");
        console.log(imgUrl)
      }, [])
    ...
    return (
        <Center py={6} _hover={{ transform: "translateY(-10px)" }} transition="0.3s">
            <Box width={'27vw'}  bg={useColorModeValue('white', 'gray.900')} boxShadow={'xl'} rounded={'xl'} p={6} overflow={'hidden'}>
                <Box h={'18vw'} overflow={"hidden"} bg={'gray.100'} mt={-6} mx={-6} mb={6} pos={'relative'} style={{backgroundImage: `url(${cardData.thumbnail})`,backgroundSize:'cover'  , backgroundRepeat:'no-repeat'}}>
